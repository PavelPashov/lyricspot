...
class Lyrics(object):
    ...
    def find_song_musixmatch(self, song, artists):
        ...
        try:
            ...
            data = response.json()
            track_url = data["message"]["body"]["track_list"][0]["track"][
                "track_share_url"
            ]
    ...
    def find_song_genius(self, song, artist):
        ...
        try:
            # Sanitize the song between putting it through the API
            for char in ["`", "'", "("]:
                if char in song:
                    song = song.replace(char, "-")
            song = "".join(song.split("-")[0])
            ...
            for track in data["response"]["hits"]:
                # Different options to find a match
                find_in_song = (
                    song.lower().strip().find(track["result"]["title"].lower().strip())
                )
                find_in_track = (
                    track["result"]["full_title"]
                    .lower()
                    .strip()
                    .find(song.lower().strip())
                )
                find_artist = (
                    artist.lower()
                    .strip()
                    .find(track["result"]["primary_artist"]["name"].lower().strip())
                )
                artist_find = (
                    track["result"]["primary_artist"]["name"]
                    .lower()
                    .strip()
                    .find(artist.lower().strip())
                )
                # Not returning -1 means there's a match
                if (find_in_song != -1 or find_in_track != -1) and (
                    find_artist != -1 or artist_find != -1
                ):

                    return track["result"]["path"]
                print(find_in_song, find_in_track, find_artist, artist_find)
        except Exception as e:
            logging.error(e, exc_info=True)
    ...
    def get_song_lyrics(self):
        ...
        google_href = "https://www.google.com/search?q=" + quote(
            f"{self.name} {self.artists[0]['name']} lyrics"
        )
        lyrics_not_found = (
    ...
    def check_for_cached_lyrics(self):
        ...
        if (
            session["lyrics"]["name"] == self.name
            and session["lyrics"]["artist"] == self.artists[0]["name"]
        ):
