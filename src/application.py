...
api.add_resource(
    RecentSongLyricsAPI, "/api/v0/songs/recetly_played/lyrics/<string:song_id>"
)
api.add_resource(RecentSongsAPI, "/api/v0/songs/recetly_played/")
api.add_resource(TopSongAPI, "/api/v0/songs/top/<string:song_id>")
api.add_resource(TopSongsAPI, "/api/v0/songs/top/term/<string:term>")
...
@refresh_token
def top_songs():
    songs = TopSongsAPI().get("long_term")["songs"]
...
def check():
    if request.method == "POST":
        try:
            ...
            if (
                check_progress(session["coll_id"]) == 1
                and session["is_published"] is False
            ):
